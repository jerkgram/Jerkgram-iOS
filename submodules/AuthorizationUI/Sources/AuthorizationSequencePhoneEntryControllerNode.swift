...
private let ghostBaseSafeLoginEnabledKey = "jerkgram.SafeLogin.GhostModeEnabled"
...
private func ghostBaseSafeLoginButtonTitle(_ enabled: Bool, strings: PresentationStrings) -> String {
    return strings.jerkgram.authGhostModeStatus(enabled: enabled)
}
final class AuthorizationSequencePhoneEntryControllerNode: ASDisplayNode {
    ...
    init(sharedContext: SharedAccountContext, account: UnauthorizedAccount?, strings: PresentationStrings, theme: PresentationTheme, debugAction: @escaping () -> Void, hasOtherAccounts: Bool) {
        ...
        self.ghostBaseSafeLoginInfoNode.attributedText = NSAttributedString(string: strings.jerkgram.authGhostModeHint, font: Font.regular(13.0), textColor: self.theme.list.itemPrimaryTextColor.withAlphaComponent(0.65))

        self.ghostBaseSafeLoginNode = SolidRoundedButtonNode(title: ghostBaseSafeLoginButtonTitle(ghostBaseInitialSafeLoginEnabled, strings: strings), theme: SolidRoundedButtonTheme(theme: self.theme), glass: false, height: 50.0, cornerRadius: 50.0 * 0.5)
        ...
        self.ghostBaseSafeLoginNode.pressed = { [weak self] in
            ...
            strongSelf.ghostBaseSafeLoginNode.title = ghostBaseSafeLoginButtonTitle(strongSelf.ghostBaseSafeLoginEnabled, strings: strongSelf.strings)
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let botLoginSize = self.ghostBaseBotLoginNode.measure(
            CGSize(
        ...
        // MARK: Jerkgram v1.2L BUILD123_SAFE_LOGIN_ALL_ACCOUNTS1
        ...
        let jerkgramOfficialAuthorizationBottomY = layout.size.height - insets.bottom - additionalBottomInset
...
// MARK: Jerkgram v1.2N BUILD125_AUTH_BOT_LOGIN_LOCALIZATION2
