...
public final class AuthorizationSequencePhoneEntryController: ViewController, MFMailComposeViewControllerDelegate, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    // MARK: GhostBase v1.0ZF BOTSAFE2 first bot login
            ...
            controller.loginWithPassword = {
                ...
                self.ghostBaseBotAuthorizationDisposable.set((
                    ...
                    error: {
                        ...
                        case .alreadyAdded:
                            text = self.presentationData.strings.jerkgram.botAlreadyAdded

                        case .invalidToken:
                            controller.passwordIsInvalid()
                            text = self.presentationData.strings.jerkgram.botInvalidToken

                        case .floodWait:
                            text = self.presentationData.strings.jerkgram.botFloodWait
                        ...
                        case .signUpRequired:
                            text = self.presentationData.strings.jerkgram.botSignUpRequired

                        case let .rpc(code):
                            text = self.presentationData.strings.jerkgram.botRpcRejected(code)
                        ...
                        controller.present(
                            textAlertController(
                                sharedContext: self.sharedContext,
                                title: self.presentationData.strings.jerkgram.botLoginTitle,
                                text: text,
                                actions: [
                                    TextAlertAction(
                                        type: .defaultAction,
