        ...
        |> mapToSignal { [weak self] peers, requiresStars -> Signal<ShareState, ShareControllerError> in
            ...
            case let .messages(messages):
                for peerId in peerIds {
                    ...
                    let ghostBasePostShareScheduledEnabled = ((UserDefaults.standard.object(forKey: "jerkgram.GhostMode.ScheduledSend") as? Bool) ?? false)
                    ...
                    if ghostBasePostShareScheduledEnabled {
                        ...
                        UserDefaults.standard.set(
                            UserDefaults.standard.integer(
                                forKey: "jerkgram.SH1.ShareScheduledIntercept.Count"
                            ) + 1,
                            forKey: "jerkgram.SH1.ShareScheduledIntercept.Count"
                        )
                        ...
                        UserDefaults.standard.set(
                            "\(peerId)",
                            forKey: "jerkgram.SH1.LastSharePeerId"
                        )

                        UserDefaults.standard.set(
                            messagesToEnqueue.count,
                            forKey: "jerkgram.SH1.LastShareMessageCount"
                        )

                        UserDefaults.standard.set(
                            Int(ghostBasePostShareScheduleTime),
                            forKey: "jerkgram.SH1.LastShareScheduleTime"
                        )
                    }
