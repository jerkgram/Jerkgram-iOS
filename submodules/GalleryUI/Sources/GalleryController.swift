    ...
    present: @escaping (ViewController, Any?) -> Void) -> GalleryItem?
{
    ...
    if let image = media as? TelegramMediaImage {
        if let file = image.video {
            let captureProtected = (message.containsSecretMedia && message.minAutoremoveOrClearTimeout != viewOnceTimeout) || (message.minAutoremoveOrClearTimeout == viewOnceTimeout && !(((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.OneTimeScreenshots") as? Bool) ?? false) && ((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.OneTimeScreenRecording") as? Bool) ?? false) && ((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.Enabled") as? Bool) ?? true) && message.id.peerId.namespace != Namespaces.Peer.SecretChat)) || message.paidContent != nil || (!(((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (message.isCopyProtected() || peerIsCopyProtected))
        ...
        case .instagram where webpageContent.file != nil && webpageContent.image != nil && webpageContent.file!.isVideo:
            content = NativeVideoContent(id: .message(message.stableId, webpageContent.file?.id ?? webpage.webpageId), userLocation: .peer(message.id.peerId), fileReference: .message(message: MessageReference(message), media: webpageContent.file!), imageReference: webpageContent.image.flatMap({ ImageMediaReference.message(message: MessageReference(message), media: $0) }), streamVideo: .conservative, enableSound: true, captureProtected: message.containsSecretMedia || (!(((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (message.isCopyProtected() || peerIsCopyProtected)), storeAfterDownload: nil)
        default:
            if let embedUrl = webpageContent.embedUrl, let image = webpageContent.image {
                if let file = webpageContent.file, file.isVideo {
                    content = NativeVideoContent(id: .message(message.stableId, file.fileId), userLocation: .peer(message.id.peerId), fileReference: .message(message: MessageReference(message), media: file), imageReference: mediaImage.flatMap({ ImageMediaReference.message(message: MessageReference(message), media: $0) }), streamVideo: .conservative, loopVideo: loopVideos, tempFilePath: tempFilePath, captureProtected: message.containsSecretMedia || (!(((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((UserDefaults.standard.object(forKey: "jerkgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (message.isCopyProtected() || peerIsCopyProtected)), storeAfterDownload: generateStoreAfterDownload?(message, file))
