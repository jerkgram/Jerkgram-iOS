...
private func jerkgramMainSettingsIcon(
    _ name: String
) -> UIImage? {
...
// MARK: Jerkgram v1.2D BUILD115_MAIN_SETTINGS_LOCALIZATION1
    ...
    PeerInfoScreenDisclosureItem(
        ...
        icon: jerkgramMainSettingsIcon("Chat/Context Menu/Eye"),
        action: {
            UserDefaults.standard.set(
                "ghostMode",
                forKey: "jerkgram.Settings.InitialPage"
    ...
    PeerInfoScreenDisclosureItem(
        ...
        icon: jerkgramMainSettingsIcon("Chat/Context Menu/MessageBubble"),
        action: {
            UserDefaults.standard.set(
                "messages",
                forKey: "jerkgram.Settings.InitialPage"
    ...
    PeerInfoScreenDisclosureItem(
        ...
        icon: jerkgramMainSettingsIcon("Premium/CopyProtection/NoForward"),
        action: {
            UserDefaults.standard.set(
                "protectedContent",
                forKey: "jerkgram.Settings.InitialPage"
    ...
    PeerInfoScreenDisclosureItem(
        ...
        icon: jerkgramMainSettingsIcon("Item List/Icons/Stories"),
        action: {
            UserDefaults.standard.set(
                "mediaStories",
                forKey: "jerkgram.Settings.InitialPage"
    ...
    PeerInfoScreenDisclosureItem(
        ...
        icon: jerkgramMainSettingsIcon("Chat/Context Menu/ApplyTheme"),
        action: {
            UserDefaults.standard.set(
                "appearance",
                forKey: "jerkgram.Settings.InitialPage"
    ...
    PeerInfoScreenDisclosureItem(
        ...
        icon: jerkgramMainSettingsIcon("Chat/Context Menu/FormatCode"),
        action: {
            UserDefaults.standard.set(
                "debugResearch",
                forKey: "jerkgram.Settings.InitialPage"
...
func settingsEditingItems(data: PeerInfoScreenData?, state: PeerInfoState, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction, isMyProfile: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    if case let .user(user) = data.peer {
        items[.info]!.append(PeerInfoScreenDisclosureItem(id: ItemPhoneNumber, label: .text(((UserDefaults.standard.object(forKey: "jerkgram.Appearance.HideOwnPhone") as? Bool) ?? false) ? "" : (user.phone.flatMap({ formatPhoneNumber(context: context, number: $0) }) ?? "")), text: presentationData.strings.Settings_PhoneNumber, icon: PresentationResourcesSettings.recentCalls, action: {
            interaction.openSettings(.phoneNumber)
        }))
    }
    var username = ""
