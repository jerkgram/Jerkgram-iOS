...
extension PeerInfoScreenNode {
    func openSettings(section: PeerInfoSettingsSection) {
        ...
        switch section {
        case .ghostbase:
            push(ghostBaseSettingsController(context: self.context))
        case .avatar:
                ...
                if user.botInfo != nil {
                    self.controller?.present(
                        textAlertController(
                            context: self.context,
                            updatedPresentationData: self.controller?.updatedPresentationData,
                            title: self.presentationData.strings.jerkgram.botLogoutTitle,
                            text: self.presentationData.strings.jerkgram.botLogoutText,
                            actions: [
                                TextAlertAction(
                                    type: .genericAction,
                                ...
                                TextAlertAction(
                                    type: .defaultAction,
                                    title: self.presentationData.strings.jerkgram.botLogoutAction,
                                    action: { [weak self] in
                                        guard let self else {
                                            return
                                        }
                                        // MARK: GhostBase v1.0ZH BOTDEDUPE1 clear marker on logout
                                        UserDefaults.standard.removeObject(
                                            forKey: "jerkgram.BotAccount.\(user.id.toInt64())"
                                        )
