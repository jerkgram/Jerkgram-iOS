...
private final class PeerInfoPendingPane {
    ...
    init(
        ...
        case .files:
            ...
            paneNode = visualPaneNode
        case .links:
            paneNode = PeerInfoListPaneNode(context: context, updatedPresentationData: updatedPresentationData, chatControllerInteraction: chatControllerInteraction, peerId: chatLocationPeerId, chatLocation: chatLocation, chatLocationContextHolder: chatLocationContextHolder, tagMask: .webPage, ghostBaseGlassEnabled: ghostBaseGlassEnabled)
        case .voice:
            let visualPaneNode = PeerInfoVisualMediaPaneNode(context: context, chatControllerInteraction: chatControllerInteraction, peerId: chatLocationPeerId, chatLocation: chatLocation, chatLocationContextHolder: chatLocationContextHolder, contentType: .voiceAndVideoMessages, captureProtected: captureProtected, initialFocusMessageIndex: nil, ghostBaseGlassEnabled: ghostBaseGlassEnabled)
            paneNode = visualPaneNode
        case .music:
            let visualPaneNode = PeerInfoVisualMediaPaneNode(context: context, chatControllerInteraction: chatControllerInteraction, peerId: chatLocationPeerId, chatLocation: chatLocation, chatLocationContextHolder: chatLocationContextHolder, contentType: .music, captureProtected: captureProtected, initialFocusMessageIndex: nil, ghostBaseGlassEnabled: ghostBaseGlassEnabled)
            paneNode = visualPaneNode
        case .gifs:
...
//                print(transitionFraction)
    ...
    func update(size: CGSize, sideInset: CGFloat, topInset: CGFloat, bottomInset: CGFloat, deviceMetrics: DeviceMetrics, visibleHeight: CGFloat, expansionFraction: CGFloat, presentationData: PresentationData, data: PeerInfoScreenData?, areTabsHidden: Bool, disableTabSwitching: Bool, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        // MARK: Jerkgram v1.2D BUILD115_LINKS_INNER_OWNER1
            ...
            component: AnyComponent(HorizontalTabsComponent(
                ...
                tabs: availablePanes.map { paneKey -> HorizontalTabsComponent.Tab in
                    ...
                    case .gifts:
                        ...
                        canReorder = true
                    case .ghostBaseProfileHistory:
                        content = .title(HorizontalTabsComponent.Tab.Title(text: presentationData.strings.jerkgram.profileHistoryTab, entities: [], enableAnimations: false))
                    case .ghostBasePresence:
                        content = .title(HorizontalTabsComponent.Tab.Title(text: presentationData.strings.jerkgram.presenceHistoryTab, entities: [], enableAnimations: false))
                    case .ghostBaseGiftHistory:
                        content = .title(HorizontalTabsComponent.Tab.Title(text: presentationData.strings.jerkgram.giftHistoryTab, entities: [], enableAnimations: false))
                    case .ghostBasePersonalChannel:
                        content = .title(HorizontalTabsComponent.Tab.Title(text: presentationData.strings.jerkgram.personalChannelTab, entities: [], enableAnimations: false))
                    case .polls:
