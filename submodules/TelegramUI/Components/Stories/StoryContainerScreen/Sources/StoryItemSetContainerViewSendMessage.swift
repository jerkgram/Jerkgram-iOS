...
//        component.controller()?.push(controller)
    ...
    private func getCaptionPanelView(view: StoryItemSetContainerComponent.View, peer: EnginePeer, mediaPicker: MediaPickerScreenImpl? = nil) -> TGCaptionPanelView? {
        ...
        }, getNavigationController: { [weak view] in
            guard let controller = view?.component?.controller() else {
                return nil
            }
    ...
    private func ghostBaseStoryEffectiveScheduleTime(_ scheduleTime: Int32?) -> Int32? {
        return (((UserDefaults.standard.object(forKey: "jerkgram.GhostMode.ScheduledSend") as? Bool) ?? false) && scheduleTime == nil) ? Int32(Date().timeIntervalSince1970) + 12 : scheduleTime
    }
