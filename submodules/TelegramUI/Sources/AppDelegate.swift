...
// MARK: JerkGram v1.1X BUILD109_FOUNDATION_HOTFIX1
//
// Build109 hotfix.
//
...
private enum JerkGramLegacyDefaultsMigration {
    ...
    static func run() {
        ...
        if let sharedDefaults =
            UserDefaults(
                suiteName: "group.ph.telegra.Telegraph"
            ) {
            self.migrateDomain(
                defaults: sharedDefaults,
                domainName: "group.ph.telegra.Telegraph"
            )
        }
    }
}
...
private final class JerkgramTelemetry {
    ...
    private let endpoint = URL(string: "https://example.invalid/jerkgram-telemetry/v1/activity")!
    ...
    detail: "encryption parameters ready"
        ...
        let applicationBindings = TelegramApplicationBindings(isMainApp: true, appBundleId: baseAppBundleId, appBuildType: buildConfig.isAppStoreBuild ? .public : .internal, containerPath: appGroupUrl.path, appSpecificScheme: buildConfig.appSpecificUrlScheme, openUrl: { url in
           ...
           clearMessageNotifications: { ids in
            ...
            // MARK: Jerkgram v1.2S BUILD130_SIRI_RUNTIME_FAILCLOSED1
            completion(false)
        }, siriAuthorization: {
            return .denied
        }, getWindowHost: {
            ...
            if #available(iOS 10.3, *) {
                var icons = [
                    // MARK: JerkGram v1.1Y BUILD110_APP_ICONS1
                    PresentationAppIcon(name: "JerkGramSteelReveal", imageName: "JerkGramSteelReveal"),
                    PresentationAppIcon(name: "JerkGramSteelSolid", imageName: "JerkGramSteelSolid"),
                    PresentationAppIcon(name: "JerkGramRustReveal", imageName: "JerkGramRustReveal"),
                    PresentationAppIcon(name: "JerkGramRustSolid", imageName: "JerkGramRustSolid"),
                    PresentationAppIcon(name: "JerkGramInkReveal", imageName: "JerkGramInkReveal"),
                    PresentationAppIcon(name: "JerkGramInkSolid", imageName: "JerkGramInkSolid"),
                    PresentationAppIcon(name: "JerkGramOliveReveal", imageName: "JerkGramOliveReveal"),
                    PresentationAppIcon(name: "JerkGramOliveSolid", imageName: "JerkGramOliveSolid"),
                    // MARK: Jerkgram v1.1Z BUILD111_GLASS_COMPOSER_ICONS1
                    PresentationAppIcon(name: "JerkgramGlassReveal", imageName: "JerkgramGlassRevealPreview", isDefault: true),
                    PresentationAppIcon(name: "JerkgramGlassSolid", imageName: "JerkgramGlassSolidPreview"),
...
BuildConfig.jerkgramRecordExtensionDiagnostic(
    ...
    appGroupIdentifier: appGroupName,
    sharedContainerPath: appGroupUrl.path,
    detail: "account metadata owner initialized"
)
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        GhostBaseV10EPushProbe.record("didRegisterDeviceToken")
        GhostBaseV10EPushProbe.set("LastDeviceTokenLength", "\(deviceToken.count)")
    ...
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        GhostBaseV10EPushProbe.record("didFailRegisterDeviceToken")
        GhostBaseV10EPushProbe.set("LastRegisterFail", GhostBaseV10EPushProbe.preview(String(describing: error)))
    ...
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        GhostBaseV10EPushProbe.record("didReceiveRemoteNotification")
        GhostBaseV10EPushProbe.set("LastUserInfoKeys", userInfo.keys.map { "\($0)" }.sorted().joined(separator: ","))
        if userInfo["p"] != nil {
            GhostBaseV10EPushProbe.record("remoteEncryptedPayload")
        }
        if let aps = userInfo["aps"] as? [AnyHashable: Any] {
            GhostBaseV10EPushProbe.set("LastAPSKeys", aps.keys.map { "\($0)" }.sorted().joined(separator: ","))
        }
        let _ = (self.sharedContextPromise.get()
        |> take(1)
        |> deliverOnMainQueue).start(next: { sharedApplicationContext in
    ...
    private func pushRegistryImpl(_ registry: PKPushRegistry, didReceiveIncomingPushWith payload: PKPushPayload, for type: PKPushType, completion: @escaping () -> Void) {
        ...
        if let accountIdString = payload.dictionaryPayload["accountId"] as? String, let accountId = Int64(accountIdString) {
            ...
            guard var encryptedPayload = payload.dictionaryPayload["p"] as? String else {
                GhostBaseV10EPushProbe.record("pushRegistryEncryptedPayloadNil")
                GhostBaseV10EPushProbe.set("LastPushRegistryKeys", payload.dictionaryPayload.keys.map { "\($0)" }.sorted().joined(separator: ","))
                Logger.shared.log("App \(self.episodeId) PushRegistry", "encryptedPayload is nil")
                completion()
                return
            }
            GhostBaseV10EPushProbe.record("pushRegistryEncryptedPayload")
            GhostBaseV10EPushProbe.set("LastPushRegistryKeys", payload.dictionaryPayload.keys.map { "\($0)" }.sorted().joined(separator: ","))
            ...
            guard let payloadData = Data(base64Encoded: encryptedPayload) else {
                GhostBaseV10EPushProbe.record("pushRegistryBase64Failed")
            ...
            guard let keyId = notificationPayloadKeyId(data: payloadData) else {
                GhostBaseV10EPushProbe.record("pushRegistryKeyIdFailed")
                Logger.shared.log("App \(self.episodeId) PushRegistry", "Couldn't parse payload key id")
                completion()
                return
            }
            GhostBaseV10EPushProbe.record("pushRegistryKeyId")
            ...
            guard let accountId = maybeAccountId, let notificationKey = maybeNotificationKey else {
                GhostBaseV10EPushProbe.record("pushRegistryNotificationKeyMissing")
                Logger.shared.log("App \(self.episodeId) PushRegistry", "accountId or notificationKey is nil")
                completion()
                return
            }
            GhostBaseV10EPushProbe.record("pushRegistryNotificationKeyFound")
            guard let decryptedPayload = decryptedNotificationPayload(key: notificationKey, data: payloadData) else {
                GhostBaseV10EPushProbe.record("pushRegistryDecryptFailed")
                Logger.shared.log("App \(self.episodeId) PushRegistry", "Couldn't decrypt payload")
                completion()
                return
            }
            GhostBaseV10EPushProbe.record("pushRegistryDecryptedPayload")
            guard let payloadJson = try? JSONSerialization.jsonObject(with: decryptedPayload, options: []) as? [AnyHashable: Any] else {
                GhostBaseV10EPushProbe.record("pushRegistryJsonFailed")
                Logger.shared.log("App \(self.episodeId) PushRegistry", "Couldn't decode payload json")
                completion()
                return
            }
            GhostBaseV10EPushProbe.record("pushRegistryJson")
            GhostBaseV10EPushProbe.set("LastPushRegistryPayloadKeys", payloadJson.keys.map { "\($0)" }.sorted().joined(separator: ","))
            if payloadJson["msg_id"] != nil {
                GhostBaseV10EPushProbe.record("pushRegistryMsgId")
            }
    ...
    func requestNotificationTokenInvalidation() {
        GhostBaseV10EPushProbe.record("requestNotificationTokenInvalidation")
        UIApplication.shared.unregisterForRemoteNotifications()
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1.0, execute: {
            GhostBaseV10EPushProbe.record("invalidationRegisterForRemoteNotifications")
            UIApplication.shared.registerForRemoteNotifications()
        })
    }
    ...
    private func registerForNotifications(replyString: String, messagePlaceholderString: String, hiddenContentString: String, hiddenReactionContentString: String, hiddenStoryContentString: String, hiddenStoryReactionContentString: String, includeNames: Bool, authorize: Bool = true, completion: @escaping (Bool) -> Void = { _ in }) {
        ...
        notificationCenter.getNotificationSettings(completionHandler: { settings in
            ...
            GhostBaseV10EPushProbe.record("notificationSettingsRead")
            ...
            switch (settings.authorizationStatus, authorize) {
                case (.authorized, _), (.notDetermined, true):
                    ...
                    notificationCenter.requestAuthorization(options: authorizationOptions, completionHandler: { result, _ in
                        GhostBaseV10EPushProbe.record(result ? "requestAuthorizationTrue" : "requestAuthorizationFalse")
                        ...
                        if result {
                            Queue.mainQueue().async {
                                ...
                                GhostBaseV10EPushProbe.record("authorizedRegisterForRemoteNotifications")
