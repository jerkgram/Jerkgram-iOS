...
// MARK: GhostBase v1.0S Standalone Scheduled Send
private func ghostBaseV10SApplyStandaloneSchedule(
    ...
    let counterKey =
        "jerkgram.SH2.StandaloneScheduledIntercept.Count"
    ...
    standardDefaults.set(
        "\(peerId)",
        forKey: "jerkgram.SH2.LastStandalonePeerId"
    )

    standardDefaults.set(
        Int(scheduleTime),
        forKey: "jerkgram.SH2.LastStandaloneScheduleTime"
    )
}
...
public func standaloneSendEnqueueMessages(
    ...
    let signals: [Signal<MessageResult, PendingMessageUploadError>] = messages.map { message in
        var attributes: [MessageAttribute] = []
        ghostBaseV10SApplyStandaloneSchedule(peerId: peerId, attributes: &attributes)
        var text: String = ""
        var media: [Media] = []
