...
// MARK: GhostBase v1.0ZD PROFILEINTEL1 Core
...
// MARK: GhostBase v1.0ZF PROFILEINTEL2 Core
...
public extension TelegramEngine {
    ...
    final class Peers {
        ...
        public func ghostBaseProfileIntel2Snapshot(
                ...
                case let .result(peerId):
                    ...
                    |> mapToSignal { user, inputUser -> Signal<String, NoError> in
                        ...
                        |> map { fullUser, photos -> String in
                            ...
                            let baseKey = "jerkgram.ProfileIntel2.\(self.account.peerId.toInt64()).\(peerId.toInt64())."
                            ...
                            if events.isEmpty {
                                // MARK: GhostBase v1.0ZG PROFILEINTEL2 no-change events are report-only
                                // The current report may say that nothing changed, but the timeline stays clean.
                            }
        ...
        public func ghostBaseProfileIntelProbe(
            username rawUsername: String
        ) -> Signal<String, NoError> {
        ...
        public func ghostBaseBotCapabilityProbe() -> Signal<String, NoError> {
            let localState =
                self.account.postbox.transaction {
                    ...
                    let botSafeLastEvent = UserDefaults.standard.string(
                        forKey: "jerkgram.BotSafe.LastEvent"
                    ) ?? "nil"
