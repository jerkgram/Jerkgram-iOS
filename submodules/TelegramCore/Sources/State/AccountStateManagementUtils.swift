...
private enum GhostBaseRuntimeDiagnosticsV11G {
    private static let queue = DispatchQueue(
        label: "jerkgram.RuntimeDiagnostics.V11G",
        qos: .utility
    )
    private static let key = "jerkgram.Runtime.Diagnostics.V11G"
...
// MARK: GhostBase v1.0ZC Bot account helper
func ghostBaseIsBotAccount(_ accountPeerId: PeerId) -> Bool {
    return UserDefaults.standard.bool(
        forKey: "jerkgram.BotAccount.\(accountPeerId.toInt64())"
    )
}
...
func initialStateWithDifference(postbox: Postbox, difference: Api.updates.Difference, overrideState: AuthorizedAccountState.State? = nil, resetChannelStates: Bool = false) -> Signal<AccountMutableState, NoError> {
    return postbox.transaction { transaction -> AccountMutableState in
...
func finalStateWithDifference(accountPeerId: PeerId, postbox: Postbox, network: Network, state: AccountMutableState, difference: Api.updates.Difference, asyncResetChannels: (([(peer: Peer, pts: Int32?)]) -> Void)?, shouldResetChannels: Bool = true) -> Signal<AccountFinalState, NoError> {
    var updatedState = state
    ...
    return finalStateWithUpdates(accountPeerId: accountPeerId, postbox: postbox, network: network, state: updatedState, updates: updates, shouldPoll: false, missingUpdates: false, shouldResetChannels: shouldResetChannels, updatesDate: nil, asyncResetChannels: asyncResetChannels)
}
        ...
        switch operation {
            ...
            case let .EditMessage(id, message):
                ...
                transaction.updateMessage(id, update: { previousMessage in
                    ...
                    let ghostBaseSaveEditHistory = JerkgramRetentionRuntime.shouldCapture(
                        accountPeerId: accountPeerId.toInt64(),
                        chatPeerId: id.peerId.toInt64(),
                        isSecretChat: id.peerId.namespace == Namespaces.Peer.SecretChat,
                    ...
                    if let previousAttribute = previousMessage.attributes.first(
                        where: { $0 is GhostBaseMessageAttribute }
                    ) as? GhostBaseMessageAttribute {
                        updatedAttributes.removeAll(
                            where: { $0 is GhostBaseMessageAttribute }
                    ...
                    // MARK: Jerkgram v1.2K BUILD122_EDIT_CAPTION_HISTORY1
                    ...
                    if ghostBaseSaveEditHistory,
                       previousMessage.text != message.text || previousEntities != updatedEntities {
                        ...
                        if let updatedAttribute = attribute?.withAddedEditVersion(
                            ...
                            GhostBaseRuntimeDiagnosticsV11G.record(
                                "edit-history peer=\(id.peerId.toInt64())"
                            )
                        }
                    }
