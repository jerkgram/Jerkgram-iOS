...
@objc(BroadcastUploadSampleHandler) class BroadcastUploadSampleHandler: RPBroadcastSampleHandler {
    ...
    // MARK: Jerkgram v1.2F BUILD117_BROADCAST_VISIBLE_DIAGNOSTIC1
    ...
    override public func broadcastStarted(withSetupInfo setupInfo: [String : NSObject]?) {
        guard let appBundleIdentifier = Bundle.main.bundleIdentifier, let lastDotRange = appBundleIdentifier.range(of: ".", options: [.backwards]) else {
            self.finishWithError(stage: "bundle")
            return
        }
        _ = lastDotRange
...
BuildConfig.jerkgramRecordExtensionDiagnostic(
    process: "broadcast",
    stage: "broadcastCoordination",
    appGroupIdentifier: appGroupName,
    sharedContainerPath: appGroupUrl.path,
    detail: "coordination selector ready"
)
